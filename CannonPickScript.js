define([
		...
		function update(params, env) {
			...
			if (mainCam && mouseState.down && !env.mouseConstraint) {
				...
				if(result.length){
					...
					var normal = gooRay.direction.mul(-1);
					if(params.useForceNormal){
						normal.setd(params.forceNormal[0],params.forceNormal[1],params.forceNormal[2]);
					}
					addMouseConstraint(params, env, p.x, p.y, p.z, b, normal);
				}
	...
	CannonPickScript.externals = {
		...
		description: 'Enables camera pick and drag Cannon.js objects in 3D space',
		parameters: [{
			key: 'whenUsed',
			type: 'boolean',
			'default': true
